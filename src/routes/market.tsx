...
import { generatePartnerAffiliateUrl } from "@/lib/shop-partner-schema";
...
function MarketPage() {
  ...
  const filteredProducts = useMemo(() => {
    return marketProducts.filter((p) => {
      const catMatch =
        activeCat === "all" ||
        PRODUCT_CATEGORY_MAP[p.id] === activeCat;
    ...
    <div className="pb-4">
      ...
      <section className="px-4 pt-6 pb-3">
        ...
        <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-0.5">
          <span className="font-bold text-foreground">Marketplace Afiliado Curado</span>
      ...
      <section className="px-4 pb-3">
        <div className="bg-white rounded-full px-4 py-2.5 flex items-center gap-2.5 ring-1 ring-zinc-200 shadow-sm focus-within:ring-2 focus-within:ring-purple-600 transition-all">
          <Search className="size-4 text-zinc-400 shrink-0" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Buscar por produto, marca (Nike, Asics), suplemento..."
            className="w-full bg-transparent text-xs sm:text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="size-5 rounded-full bg-zinc-100 hover:bg-zinc-200 grid place-items-center text-zinc-500 shrink-0 transition"
      ...
      {/* Categories — horizontal scrollable carousel with drag-to-scroll */}
      <section className="pb-4">
        <div className="relative">
          ...
          <div
            ...
            className="flex gap-1.5 overflow-x-auto overflow-y-hidden px-4 py-1.5 no-scrollbar select-none cursor-grab active:cursor-grabbing touch-pan-x overscroll-x-contain"
          >
            <button
              onClick={() => selectCategory("all")}
              className={`shrink-0 px-3 py-1 rounded-full text-xs font-semibold ring-1 transition-all shadow-sm ${
                activeCat === "all"
      ...
      {/* Dynamic Results Header when Search or Category Filter is active */}
      {(searchQuery || activeCat !== "all") && (
        <section className="px-4 pb-3 flex items-center justify-between">
          <p className="text-xs font-semibold text-zinc-700">
            ...
            {searchQuery && <span className="font-bold text-purple-700"> para "{searchQuery}"</span>}
          </p>
          <button
            onClick={() => {
              setSearchQuery("");
      ...
      {/* Render Filtered Results when Search or Category Filter is active */}
      {searchQuery || activeCat !== "all" ? (
        <section className="px-4 pb-8">
          {filteredProducts.length === 0 ? (
            <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-8 text-center my-2">
              <div className="size-12 rounded-full bg-purple-100 text-purple-600 grid place-items-center mx-auto mb-3">
                <Search className="size-6" />
              </div>
              <p className="text-sm font-bold text-zinc-900">Nenhum produto encontrado</p>
              <p className="text-xs text-zinc-500 mt-1 max-w-xs mx-auto leading-relaxed">
                Tente pesquisar por termos como <span className="font-semibold text-purple-700">NovaBlast, Creatina, Liquidz, Garmin, Apple, Short ou Nutrição</span>.
              </p>
            </div>
          ) : (
              ...
              {filteredProducts.map((p) => {
                ...
                return (
                  <button
                    key={p.id}
                    onClick={() =>
                      openProduct(p, "Resultado de Busca", isDouble ? "nfs em dobro" : undefined)
                    }
                    className="bg-card rounded-2xl overflow-hidden ring-1 ring-black/5 text-left active:scale-[0.98] transition-transform shadow-sm"
                  >
                    <div className="relative">
                      <img
                        src={p.image}
                      ...
                      />
                      {isDouble && (
                        <span className="absolute top-2 left-2 text-[10px] font-bold tracking-widest text-white bg-lime-500 px-2 py-0.5 rounded shadow">
                          nfs em dobro
                        </span>
                      )}
                    </div>
